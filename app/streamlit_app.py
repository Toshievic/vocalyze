...
def total_views():
    ...
    year = st.selectbox(label="リリース年を選ぶ", options=range(2007,datetime.now().year+1), index=None)
    display_stat("num_views", offset, year)
...
def total_comments():
    ...
    offset = st.number_input("OFFSET:", min_value=0, value=0, step=100)
    year = st.selectbox(label="リリース年を選ぶ", options=range(2007,datetime.now().year+1), index=None)
    display_stat("num_comments", offset, year)
...
def total_likes():
    ...
    offset = st.number_input("OFFSET:", min_value=0, value=0, step=100)
    year = st.selectbox(label="リリース年を選ぶ", options=range(2007,datetime.now().year+1), index=None)
    display_stat("num_likes", offset, year)
...
def total_mylists():
    ...
    offset = st.number_input("OFFSET:", min_value=0, value=0, step=100)
    year = st.selectbox(label="リリース年を選ぶ", options=range(2007,datetime.now().year+1), index=None)
    display_stat("num_mylists", offset, year)
...
st.session_state.realtime = st.checkbox("現在の流行曲に並べ替え")

if st.session_state.realtime:
    agg_period = st.selectbox("集計期間", ("月間","週間","日間","1時間"))
    st.session_state.is_normal = False
    st.session_state.df = get_period_stat(agg_period)
elif not st.session_state.realtime and not st.session_state.is_normal:
